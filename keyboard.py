...
class Screen(Observer):
    ...
    def update(self,model,key="C") :
        ...
        result=cursor.execute("SELECT " + key_search +  " FROM frequencies WHERE octave=" + str(model.get_degree()) + ";")
        freq=result.fetchone()[0]
        
        if not os.path.isfile(filename):
            save_note_wav(filename,freq,2*freq)

        subprocess.call(["aplay",model.get_gamme()[key]])

        if model.grap:
            model.grap.set_frequence([freq])
            print(freq)
            #print(key + str(self.model.get_degree()))
            #connect.commit()
            #print(key)
...
class Keyboard :
    ...
    def create_keyboard(self,key_w=40,key_h=150) :
        ...
        for key in self.model.gamme.keys() :
            if key.startswith('#',1,len(key)) :
                ...
                note = tk.Label(self.keyboard, text = str(key))
                note.place(x=key_w*delta_x+key_w*dx_black,y=0)
                self.label.append(note)
                self.place_Label.append((key_w*delta_x+key_w*dx_black,0)) 
                ...
                note = tk.Label(self.keyboard, text = str(key))
                note.place(x=key_w*dx_white,y=120)
                self.label.append(note)
                self.place_Label.append((key_w*dx_white,key_h-20))                    
    ...
    def play_note(self,key) :
        ...
        note = key + str(self.model.get_degree())
        print(note)
